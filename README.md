...
coin_ Вы можете заменить на что-то другое (resource_), тогда плагин будет думать, что наградой являются ресурсы. Измените reward.coins на reward.(Материал ресурса, пример: reward.EMERALD), и укажите количество ресурсов, что хотите получить в награду. Аналогичная система используется в других типах:
...
Чтобы добавить новый квест - нужно изменить код плагина. Весь код, регистрирующий выполнение квеста, находится в пакете me.degeron.questplugin.menu.event.completing.
